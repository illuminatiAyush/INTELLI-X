...
const AttendancePage = () => {
  ...
  useEffect(() => {
    const fetchBatches = async () => {
      try {
        if (role === 'student') {
          // Find the student record for this user, then get their batches
          ...
          if (studentRow) {
            ...
            const studentBatches = (links || [])
              .map(l => l.batches)
              .filter(Boolean)
              .sort((a, b) => a.name.localeCompare(b.name))
            setBatches(studentBatches)
            if (studentBatches.length > 0) setSelectedBatch(studentBatches[0].id)
          }
        } else {
          ...
          <div className="space-y-3">
            {students.map((student, i) => (
              <motion.div
                ...
                className={`flex items-center justify-between px-6 py-4 rounded-2xl border transition-all ${attendance[student.id] === 'present'
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shadow-inner ${attendance[student.id] === 'present'
                ...
                {canEdit ? (
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all active:scale-95 ${attendance[student.id] === 'present'
                  ...
                  <div className={`px-4 py-2 rounded-xl text-xs font-bold border ${attendance[student.id] === 'present'
          ...
          {canEdit && (
            <div className="flex justify-end pt-6">
              <motion.button
                ...
                onClick={handleSave}
                disabled={saving}
                className={`flex items-center gap-2 px-8 py-3 rounded-2xl text-sm font-bold shadow-xl transition-all ${saved
