...
import { Eye, EyeOff, ArrowRight, Mail, Lock, Sun, Moon, User, Building2, GraduationCap, Briefcase, BookOpen, Phone } from 'lucide-react'
...
const RegisterPage = () => {
  ...
  const [form, setForm] = useState({ fullName: '', email: '', password: '', phone: '', role: 'student', instituteCode: '', subject: '' })
  ...
  const validate = () => {
    ...
    if (form.role === 'teacher' && !form.subject.trim()) e.subject = 'Subject is required for teachers'
    return e
  }
          ...
          {submitted ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-4">
                ...
                <div>
                  <div className="relative">
                    <Phone className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
                    <input
                      name="phone"
                      type="tel"
                      value={form.phone}
                      ...
                      className={`w-full pl-10 pr-4 py-3.5 rounded-xl border text-sm outline-none transition-all duration-200 ${inputBg} ${errors.phone ? 'border-red-500/60 focus:border-red-500' : ''}`}
                    />
                  </div>
                  {errors.phone && (
                    <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="mt-1.5 text-xs text-red-400">{errors.phone}</motion.p>
                  )}
                </div>
